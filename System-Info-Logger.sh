...
RAM(){
    ...
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "Total RAM" "${TOTAL_RAM}GB"
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "Total Free Space" "${FREE_RAM_SPACE}GB"
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "Total Used Space" "${USED_RAM_SPACE}GB"
    PRINT_TABLE_ROW "-----------" "-----------" "-----------"
}
...
STORAGE(){
    ...
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "Total Storage Size" "${TOTAL_STORAGE_SPACE}GB"
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "Total Available Storage" "${AVAILABLE_STORAGE_SPACE}GB"
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "Total Used Storage" "${USED_STORAGE_SPACE}GB"
    PRINT_TABLE_ROW "-----------" "-----------" "-----------"
}
...
OS(){
    ...
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "OS Name" "${OS_NAME}"
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "OS Version" "${OS_VERSION}"
    PRINT_TABLE_ROW "-----------" "-----------" "-----------"
}
...
IP_ADDRESS(){
    ...
    PRINT_TABLE_ROW "${CURRENT_DATE} $(date +%T.%3N)" "IP Address" "${IP}"
}
